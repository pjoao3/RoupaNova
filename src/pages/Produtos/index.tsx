...
<View style={{height: 150, marginTop: 15, flexDirection: "row"}}>
  <Image source={item.image} style={{height: 150, width: 150, borderRadius: 15}} />
  <View style={{ flex: 1, paddingLeft: 10}}>
    <Text style={{fontSize: 18, fontWeight: "bold", marginBottom: 5, color: "#8c52ff", marginRight: 15}}>{item.title}</Text>
    <Text style={{color: "#8c52ff", marginRight: 15}}>{item.description}</Text>
    <Text style={{marginTop: 20, fontSize: 18, fontWeight: "bold", color: "#8c52ff", marginRight: 15}}>{item.price}</Text>
    <Text style={{color: "#8c52ff", marginRight: 15}}>{item.date}</Text>
  </View>
</View>
<View style={{height: 1, backgroundColor: "#8c52ff", marginTop: 15, marginRight: 15}} />
</>
);
...
const Produtos: React.FC<any> = ({ route }) => {
  ...
  return (<>
    <View
    style={{height:getStatusBarHeight()+15, backgroundColor:'#8c52ff'}}/>
  ...
  <View style={{flex: 1, paddingLeft: 15, backgroundColor: "white"}}>
    ...
    <Text style={{fontSize: 32, fontWeight: "bold", marginTop: 32, color: "#8c52ff"}}>Olá{nome? ` ${nome}`:""}!</Text>
    <Text style={{fontSize: 18, color: "#8c52ff", marginBottom: 15}}>Conheça os nosso produtos abaixo:</Text>
    <FlatList
        data={DATA}
        renderItem={Item}
